...
exports.getContestantById = exports.getContestants = void 0;
...
const getContestants = async (req, res) => {
    try {
        ...
        const contestantsWithStats = await Promise.all(users.map(async (user, index) => {
            // Get user's submissions
            const submissions = await Submission_1.default.find({ userId: user._id });
            // Calculate total votes from all submissions
            const submissionIds = submissions.map(sub => sub._id);
            const votesBySubmission = await Vote_1.default.find({ submissionId: { $in: submissionIds } });
            const votesByContestant = await Vote_1.default.find({ contestantId: user._id });
            const allVotes = [...votesBySubmission, ...votesByContestant];
            // Remove duplicates (votes that match both criteria)
            const uniqueVotes = allVotes.filter((vote, index, self) => index === self.findIndex(v => v._id.toString() === vote._id.toString()));
...
const getContestantById = async (req, res) => {
    try {
        ...
        const submissionsWithVotes = await Promise.all(submissions.map(async (submission) => {
            const votes = await Vote_1.default.find({ submissionId: submission._id });
        ...
        // Get rank
        const allUsers = await User_1.default.find({ role: { $ne: 'admin' } }).select('_id');
        const voteCounts = await Promise.all(allUsers.map(async (u) => {
            const userSubmissions = await Submission_1.default.find({ userId: u._id });
            const submissionIds = userSubmissions.map(sub => sub._id);
            const userVotes = await Vote_1.default.find({ submissionId: { $in: submissionIds } });
