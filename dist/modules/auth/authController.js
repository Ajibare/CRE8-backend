...
exports.completeRegistration = exports.refreshToken = exports.logout = exports.resetPassword = exports.forgotPassword = exports.updateProfile = exports.getProfile = exports.login = exports.register = void 0;
    ...
    try {
        ...
        if (existingUser) {
            ...
            if (!existingUser.password) {
                ...
                existingUser.password = await (0, passwordUtils_1.hashPassword)(password);
        ...
        try {
            await (0, sendEmail_1.sendEmail)(user.email, 'Welcome to FUNTECH Creative Innovation!', sendEmail_1.emailTemplates.welcome(user.name, user.creativeId));
...
const updateProfile = async (req, res) => {
    try {
        ...
        const user = await User_1.default.findByIdAndUpdate(req.user._id, { bio, socialLinks }, { new: true, runValidators: true }).select('-password');
        res.json({
...
const forgotPassword = async (req, res) => {
    try {
        ...
        const resetToken = (0, passwordUtils_1.generateResetToken)();
        const resetTokenHash = (0, passwordUtils_1.generateResetTokenHash)(resetToken);
    ...
    `;
        ...
        try {
            await (0, sendEmail_1.sendEmail)(user.email, 'Password Reset Request - FUNTECH Creative Challenge', resetEmailHtml);
...
const resetPassword = async (req, res) => {
    try {
        ...
        const tokenHash = (0, passwordUtils_1.generateResetTokenHash)(token);
        ...
        if (user.password) {
            const isSamePassword = await (0, passwordUtils_1.comparePassword)(newPassword, user.password);
            if (isSamePassword) {
                return res.status(400).json({
    ...
    `;
        await (0, sendEmail_1.sendEmail)(user.email, 'Password Reset Successful - FUNTECH Creative Challenge', confirmationEmailHtml);
        res.json({
...
const refreshToken = async (req, res) => {
    try {
        ...
        const newToken = jsonwebtoken_1.default.sign({ userId: user._id }, process.env.JWT_SECRET || 'fallback-secret', { expiresIn: '7d' });
        res.json({
...
const completeRegistration = async (req, res) => {
    try {
        ...
        if (!user.creativeId) {
            user.creativeId = await (0, generateCreativeId_1.generateCreativeId)();
        }
        user.isVerified = true;
        await user.save();
        // Send welcome email
        await (0, sendEmail_1.sendEmail)(user.email, 'Welcome to FUNTECH Creative Challenge!', sendEmail_1.emailTemplates.welcome(user.name, user.creativeId));
        res.json({
