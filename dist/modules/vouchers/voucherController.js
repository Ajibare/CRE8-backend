...
exports.getAllVouchers = exports.completeRegistrationWithVoucher = exports.applyVoucherToRegistration = exports.createVoucher = exports.validateVoucher = void 0;
const Voucher_1 = __importDefault(require("../../database/models/Voucher"));
...
const validateVoucher = async (req, res) => {
    try {
        ...
        // Validate voucher
        const validation = await Voucher_1.default.validateVoucher(code, type);
        if (!validation.valid) {
...
const createVoucher = async (req, res) => {
    try {
        const { discount, type = 'REGISTRATION', expiryDays = flutterwave_1.voucherConfig.expiryDays } = req.body;
        if (!discount || discount <= 0) {
            return res.status(400).json({ message: 'Valid discount amount is required' });
        }
        // Generate unique 6-digit voucher code
        const code = Voucher_1.default.generateCode();
        ...
        const voucher = new Voucher_1.default({
            ...
            type,
            expiresAt,
            createdBy: req.user._id // Assuming admin user
        });
        await voucher.save();
        res.status(201).json({
...
const applyVoucherToRegistration = async (req, res) => {
    try {
        ...
        // Validate voucher
        const validation = await Voucher_1.default.validateVoucher(voucherCode, 'REGISTRATION');
        if (!validation.valid) {
            return res.status(400).json({
                message: validation.message,
                valid: false
...
exports.applyVoucherToRegistration = applyVoucherToRegistration;
// Complete registration with voucher
const completeRegistrationWithVoucher = async (req, res) => {
    try {
    ...
    `;
        await (0, sendEmail_1.sendEmail)(user.email, 'Registration Successful - FUNTECH Creative Challenge', confirmationEmailHtml);
        res.json({
...
exports.completeRegistrationWithVoucher = completeRegistrationWithVoucher;
