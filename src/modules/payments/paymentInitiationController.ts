...
import { paymentTypes, paystackConfig } from '../../config/paystack';
...
export const handlePaystackCallback = async (req: Request, res: Response) => {
  try {
    ...
    const paystackData: any = response.data;
    ...
    try {
      const amount = paystackData.data.amount || paymentTypes.REGISTRATION;
      await sendEmail(
        email,
        'Payment Confirmation - FUNTECH Creative Challenge',
        emailTemplates.paymentConfirmation(email, String(reference), amount)
      );
...
export const initiateFlutterwaveRegistrationPayment = async (req: Request, res: Response) => {
  try {
    const { email, referralCode } = req.body;
    ...
    amount = paymentTypes.REGISTRATION;
...
export const handleFlutterwaveCallback = async (req: Request, res: Response) => {
  try {
    ...
    const flutterwaveData: any = response.data;
    ...
    // Send payment confirmation email
    try {
      const amount = flutterwaveData.data.amount || paymentTypes.REGISTRATION;
      await sendEmail(
        email,
        'Payment Confirmation - FUNTECH Creative Challenge',
        emailTemplates.paymentConfirmation(email, String(reference), amount)
